...
const (
	...
	rbdSubsystem = "rbd_mirroring"
)

var _ prometheus.Collector = &CephBlockPoolCollector{}

// CephRBDMirroringCollector is a custom collector for CephBlockPool Custom Resource
...
// NewCephRBDMirroringCollector constructs a collector
func NewCephBlockPoolCollector(opts *options.Options) *CephBlockPoolCollector {
	...
	return &CephBlockPoolCollector{
		MirroringImageHealth: prometheus.NewDesc(
			prometheus.BuildFQName(namespace, rbdSubsystem, "image_health"),
			`RBD Mirroring Image Health. 0=OK, 1=WARNING & 2=ERROR`,
			[]string{"name", "namespace"},
			nil,
		),
		MirroringStatus: prometheus.NewDesc(
			prometheus.BuildFQName(namespace, rbdSubsystem, "status"),
			`RBD Mirroring Image Health. 0=Enabled, 1=Disabled`,
			[]string{"name", "namespace"},
			nil,
		),
	...
	for _, cephBlockPool := range cephBlockPools {
		...
		case "ERROR":
			ch <- prometheus.MustNewConstMetric(c.MirroringImageHealth,
				...
				cephBlockPool.Namespace)
		default:
			klog.Errorf("Ceph RBD mirroring image in unexpected health status. Must be OK, WARNING or ERROR")
		}
	}
}
	...
	for _, cephBlockPool := range cephBlockPools {
		...
		case false:
			ch <- prometheus.MustNewConstMetric(c.MirroringStatus,
				...
				cephBlockPool.Namespace)
		default:
			klog.Errorf("Ceph mirroring in unexpected status. Must be true, false")
		}
	}
}
